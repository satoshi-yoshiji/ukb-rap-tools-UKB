...
data_file_dir="/03.sex_stratified_BMI/ap_imp37_gwas_male/"
...
for chr in {1..22}; do
  run_regenie_cmd="regenie --step 2 --out assoc.c${chr} \
    ...
    --phenoCol BMI \
    --pred ap37_results_pred.list --bsize 200 \
  ...
  dx run swiss-army-knife -iin="${data_file_dir}/${data_field}_c${chr}_v3.bed" \
   ...
   -iin="${gwas_file_dir}/ap37_results_pred.list" \
   -iin="${gwas_file_dir}/ap37_results_1.loco.gz" \
   -icmd="${run_regenie_cmd}" --tag="Step3" --instance-type "mem1_ssd1_v2_x16"\
